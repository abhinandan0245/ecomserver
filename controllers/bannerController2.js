// ✅ Sequelize Model
const {Banner2} = require('../models');

//  Create Banner
exports.createBanner2 = async (req, res) => {
  try {
    ...
    const banner = await Banner2.create({
      title,
      description,
...
//  Get All Banners
exports.getAllBanners2 = async (req, res) => {
  try {
    const banners = await Banner2.findAll(); // simple fetch
...
// Get Banner by ID
exports.getBannerById2 = async (req, res) => {
  try {
    const banner = await Banner2.findByPk(req.params.id);
    if (!banner) {
...
// ✅ Update Banner
exports.updateBanner2 = async (req, res) => {
  try {
    const { title, description, linkUrl, isActive} = req.body;
    const banner = await Banner2.findByPk(req.params.id);
...
exports.toggleBannerStatus2 = async (req, res) => {
  try {
    const banner = await Banner2.findByPk(req.params.id);
